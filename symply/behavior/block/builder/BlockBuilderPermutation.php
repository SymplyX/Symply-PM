...
final class BlockBuilderPermutation extends BlockBuilder{
	...
	public function getPropertiesTag() : CompoundTag
	{
		return parent::getPropertiesTag()
			->setTag("permutations", new ListTag(array_map(fn(BlockPermutation $permutation) => $permutation->toNBT(), $this->getPermutations())))
			->setTag("properties", new ListTag(array_reverse(array_map(fn(BlockProperty $property) => $property->toNBT(), $this->getProperties()))));// TODO: Change the autogenerated stub
	}
	...
	public function toBlockStateDictionaryEntry() : Generator
	{
		...
		$listBlockPropertyName = array_map(fn(BlockProperty $property) => $property->getName(), $properties);
		foreach (Utils::getCartesianProduct(array_map(fn(BlockProperty $property) => $property->getValueInRaw(), $properties)) as $meta => $property) {
			$states = [];
